...
def train(model, criterion, optimizer, pos_feats, neg_feats, maxiter, in_layer='fc4'):
    ...
    for iter in range(maxiter):
        ...
        # forward
        pos_score = model(batch_pos_feats)
        neg_score = model(batch_neg_feats)
...
def run_mdnet(img_list, init_bbox, gt=None, savefig_dir='', display=False):
    ...
    model.load_state_dict(torch.load(opts['model_path'])['layers'])
    if opts['use_gpu']:
        model = model.cuda()
    ...
    neg_examples = np.concatenate([
                    gen_samples(SampleGenerator('uniform', image.size, 1, 2, 1.1), target_bbox, opts['n_neg_init']//2, opts['overlap_neg_init']),
                    gen_samples(SampleGenerator('whole', image.size, 0, 1.2, 1.1), target_bbox, opts['n_neg_init']//2, opts['overlap_neg_init'])])
    ...
    for i in range(1, len(img_list)):
        ...
        if success:
            # Draw pos/neg samples
            pos_examples = gen_samples(pos_generator, target_bbox, opts['n_pos_update'], opts['overlap_pos_update'])
            neg_examples = gen_samples(neg_generator, target_bbox, opts['n_neg_update'], opts['overlap_neg_update'])
        ...
        if gt is None:
            ...
            print("Frame %d/%d, Overlap %.3f, Score %.3f, Time %.3f" % (i, len(img_list), overlap_ratio(gt[i], result_bb[i])[0], target_score, spf))
